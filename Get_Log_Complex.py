...
def get_super_log_and_log(model_location, split_frac):
    ...
    save_log(super_log, "Super_Log.csv")
    
    split_point = math.floor(len(super_log) * split_frac)
    samp_log = super_log[0:split_point]
    samp_log = samp_log * (1/split_frac)
    save_log(samp_log, "Samp_Log.csv")
    
    samp_variants = get_variants_list(samp_log)
    super_test = remove_variants(super_log, samp_variants)
    save_log(super_test, "Super_Test.csv")
